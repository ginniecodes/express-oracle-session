...
module.exports = function(session) {
  ...
  PRIMARY KEY (${this.options.schema.columnNames.session_id})
		...
		this.getConnection(function(conn) {
            conn.execute(sql, function(error) {
                if (error) {
	...
	oracleDbStore.prototype.get = function(session_id, cb) {
		...
		this.getConnection(function(conn) {
            conn.execute(sql, params, { outFormat: oracledb.ARRAY, fetchInfo: { "DATA": { type: oracledb.STRING} } }, // Fetch as a String instead of a Stream
...
VALUES (:sessionid,:expires,:attributes)`
		...
		this.getConnection(function (conn) {
            conn.execute(sql, params, function(error) {
